...
> [!WARNING]
> Докер может слишком долго подключаться к Oracle, изз-за чего VSCode будет выдавать ошибку подключения. Контейнер Oracle, когда полностью готов к использованию, пишет в логах следующее сообщение
>```
>#########################
