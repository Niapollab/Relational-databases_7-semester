...
4. Дождитесь завершения построения контейнера. В журнал будет выведено сообщение:
...
### Запросы блокируются/завершаются с ошибкой, что делать?

> [!TIP]
> Чаще всего это происходит из-за того, что **соединение уже открыто в другом файле**. Если вы выполняете запросы в одном файле, а затем пытаетесь сделать то же самое в другом, предыдущее соединение остаётся активным. В результате новые запросы могут блокироваться или завершаться с ошибкой. **Закройте предыдущее соединение в файле, где оно было открыто**.
